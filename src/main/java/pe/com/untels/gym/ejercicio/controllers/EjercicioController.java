...
public class EjercicioController {
    ...
    public ResponseEntity<EjercicioDTO> registrar(@RequestBody EjercicioInsertDTO request) {
        ModelMapper modelMapper = new ModelMapper();
        Ejercicio ejercicio = modelMapper.map(request, Ejercicio.class);
        EjercicioDTO response = modelMapper.map(ejercicioService.insert(ejercicio), EjercicioDTO.class);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    // FIX: agregado — faltaba edición
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> actualizar(@PathVariable Integer id, @RequestBody EjercicioInsertDTO request) {
        ...
        // OJO: idGrupoMuscular viene como Integer en el DTO, no como objeto GrupoMuscular.
        // Hay que resolverlo en el service antes de guardar, igual que en insert().
