...
public class GrupoMuscularController {
    ...
    @Autowired
    private IEjercicioRepository ejercicioRepositorio;
    @GetMapping("/lista")
    ...
    public ResponseEntity<String> eliminar(@PathVariable int id) {
        ...
        long cantidadEjercicios = ejercicioRepositorio.countByGrupoMuscular_IdGrupo(id);
        if (cantidadEjercicios > 0) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    "No se puede eliminar: hay " + cantidadEjercicios +
                            " ejercicio(s) asociados a este grupo muscular. Reasígnalos o elimínalos primero."
            );
        }

        grupoMuscularService.delete(id);
        return ResponseEntity.ok("Grupo muscular eliminado correctamente");
    }
}